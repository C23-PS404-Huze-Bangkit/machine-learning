...
def get_same_or_newer(start_date, data):
    ...
    # min_date_employees = []
    sorted_data = sorted(reader, key=lambda row: row[3])
    # datetime.strptime(row[0], '%m/%d/%Y')
    # filtered_data = [row for row in sorted_data if datetime.datetime.strptime(row[3], '%Y-%m-%d') > start_date]
    # min_date_employees.append("{} {}".format(filtered_data[0], filtered_data[1]))
    # print(filtered_data)
...
def list_newer(start_date):
    ...
    list_employees = get_same_or_newer(start_date, data)
    for row in list_employees:
        print("Started on {}: {} {}".format(datetime.datetime.strptime(row[3], "%m %d, %Y"), row[0], row[1]))
